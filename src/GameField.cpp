...
#define BUBBLE_SIZE 40 /* размер мишени в пиксел€х */
...
void GameField::Init() {
  ...
  /* ћаксимальное увеличение скорости снар€да - в 3 раза больше указанной в настройках */
...
void GameField::Shoot(const IPoint &mouse_pos, const int gunStrength) {
  /* ѕусть будет снар€дов на поле будет не больше указанного количества */
...
void GameField::CheckBullets() {
  ...
  for each (BulletPtr bullet in _bullets) {
    /* ”дал€ем попавшие в стенки снар€ды */
    if (!bullet->IsInFly()) {
      objectIdToDelete.push_back(bullet->Id());
      continue;
    }
    /* ѕровер€ем пересечение снар€дов и мишеней */
    for each (BubblePtr bubble in _bubbles) {
